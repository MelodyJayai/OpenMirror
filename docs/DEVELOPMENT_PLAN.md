...
**当前实现进度**：M0、M1、M2 已完成；M3 已完成 pair-setup/pair-verify、SETUP 解析及媒体端口分配，并集成 GPL PlayFair/SAPv2.5 的无宿主导入 WebAssembly provider，可完成四模式 fp-setup、72 字节 `ekey` → 16 字节媒体密钥解包，以及 legacy pairing 下 `SHA-256(mediaKey ‖ X25519Secret)` 会话密钥绑定。M4 已具备镜像 TCP 帧增量解析（含真实 IDR `00 10` 类型/标志拆分）、AES 视频/音频解密器、H.264 avcC 参数集解析、SPS 与 codec 头源尺寸/方向识别、AVCC→Annex-B 转换、音频 RTP 解包/重排序、发送端 `controlPort` 上的 `0x55/0x56` 批量限次丢包重传与大序列跳变自愈、向发送端 `timingPort` 主动采样的 NTP 时钟、0x54 音频同步包解析、远端 RTP/NTP/视频时间戳到本地呈现时间的统一映射、eventPort 反向 HTTP/bplist 事件通道，以及 `packages/media` 中按共享时钟调度的 ffplay H.264 视频与 AAC-ELD/RFC 3640 音频输出。AAC-ELD 解码路径已用 FFmpeg 官方 44.1 kHz ELD 测试向量验证，接收器不再对无固定魔数的原始 ELD access unit 使用首字节白名单；CLI 现可输出不含 IPv4/IPv6 地址、密钥和媒体负载的分运行 JSONL 互操作报告，并在 FLUSH、发送端锁屏暂停、媒体静默、视频通道关闭、RTSP 异常断线和分辨率/方向变化时回收/重建播放器。M5 的 RAOP 纯音频控制面已完成：RFC 4566 SDP `ANNOUNCE` 解析（AppleLossless/mpeg4-generic/L16 rtpmap、11 字段 ALAC fmtp、`a=fpaeskey`/`a=aesiv` 密钥材料、延迟属性）、legacy `Transport` 头 SETUP（`control_port`/`timing_port` → `server_port` 应答与 `Audio-Jack-Status`）、`RECORD` RTP-Info、`SET_PARAMETER` 音量/进度/DMAP 元数据/封面与 `GET_PARAMETER` 音量回读，密钥沿用 FairPlay provider 解包路径（不支持也不内置 RSA `et=1`，ANNOUNCE 遇 `rsaaeskey` 返回 453），未加密 RAOP 音频事件正确标记明文；CLI 会记录 ANNOUNCE/音量事件并对暂不支持本地播放的 ct（ALAC/PCM）只收流不开播放器。M5 的 Electron 桌面壳已有首个可运行版本（`apps/desktop`，`npm run start:desktop`）：主进程托管 AirPlayReceiver 并通过 IPC 把解密后的 H.264 Annex-B 送渲染进程用 WebCodecs `VideoDecoder` 解码上屏，含系统托盘、常驻后台、设置窗口（设备名/端口，JSON 持久化并可热重启接收器）与单实例锁；CI 通过 `ELECTRON_SKIP_BINARY_DOWNLOAD` 跳过 Electron 二进制下载保持矩阵轻量。**尚未完成**：跨多款 iPhone/iPad/iOS 版本的持续真机兼容矩阵与延迟调优、单进程帧级音画同步、RAOP ALAC/PCM 本地回放（ffmpeg 无 ALAC RTP depacketizer，需自带解码或转封装）、桌面端音频输出（Chromium 不带 AAC-ELD 解码器，待统一 WebAudio 媒体图）、多显示器/PIN 设置项与三平台安装包打包。
